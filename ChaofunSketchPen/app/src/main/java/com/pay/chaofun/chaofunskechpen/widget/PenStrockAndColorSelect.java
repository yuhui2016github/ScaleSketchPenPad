...
import com.pay.chaofun.chaofunskechpen.R;
...
public class PenStrockAndColorSelect extends RelativeLayout {
    ...
    private int CURRENT_TYPE = STROCK_TYPE;
    ...
    public void initialize(Context context) {
        ...
        final int height = 90;
        final int margin = 13;
        final int length = MarkPath.m_penColors.length;
        int left = 10;

        for (int i = 0; i < length; ++i) {
            int color = MarkPath.m_penColors[i];
            ...
            if (CURRENT_TYPE == STROCK_TYPE) {
                ...
                paint.setStrokeWidth(MarkPath.m_PenStrock[i]);
    ...
    public void setStrockORColor(int type) {
        try {
            CURRENT_TYPE = type;
            postInvalidateDelayed(200);
        } catch (Exception e) {
            Log.e("非法输入", "必须传入COLOR_TYPE 或 STROCK_TYPE");
        }
    }
